...
do
    ...
    bsub -N -B -R stable -n ${cpus} -J weak_chain_stable -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli/alsuqcli --multi-y ${cpus} kelvinhelmholtz/kelvinhelmholtz.xml

    cd $cwd
    cd runs/multix/2d/weak_scaling/kelvinhelmholtz_${N}/
    bsub -N -B -R stable -n ${cpus} -J weak_chain_stable -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli/alsuqcli --multi-x ${cpus} kelvinhelmholtz/kelvinhelmholtz.xml

    cd $cwd
    cd runs/multixmultiy/2d/weak_scaling/kelvinhelmholtz_${N}/
    x_cpus=$(($N/${BASE_N}))
    bsub -N -B -R stable -n ${cpus} -J weak_chain_stable -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli/alsuqcli --multi-x ${x_cpus} --multi-y ${x_cpus} kelvinhelmholtz/kelvinhelmholtz.xml
done;

cd $cwd;
cd notebooks; 
bsub -J weak_chain_stable -w "done(weak_chain_stable)"  -N -B $HOME/.local/bin/jupyter nbconvert --ExecutePreprocessor.timeout=-1 --to notebook --execute WeakScalingReport.ipynb --output ../reports/WeakScalingReport_euler_stable_$(date +%Y%m%d).ipynb
