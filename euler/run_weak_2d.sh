...
do
    ...
    bsub -n ${cpus} -J weak_chain -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli --multi-y ${cpus} kelvinhelmholtz/kelvinhelmholtz.xml

    cd $cwd
    cd runs/multix/2d/weak_scaling/kelvinhelmholtz_${N}/
    bsub -n ${cpus} -J weak_chain -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli --multi-x ${cpus} kelvinhelmholtz/kelvinhelmholtz.xml

    cd $cwd
    cd runs/multixmultiy/2d/weak_scaling/kelvinhelmholtz_${N}/
    x_cpus=$(($N/${BASE_N}))
    bsub -n ${cpus} -J weak_chain -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli --multi-x ${x_cpus} --multi-y ${x_cpus} kelvinhelmholtz/kelvinhelmholtz.xml
done;
