...
do
    ...
    bsub -N -B -R beta -n ${cpus} -J weak_chain_beta_without_write -W 24:00 mpirun -np ${cpus} ${cwd}/alsvinn/build/alsuqcli/alsuqcli --multi-y ${cpus} kelvinhelmholtz/kelvinhelmholtz.xml

done;

cd $cwd;
cd notebooks; 
bsub -J weak_chain_beta_without_write_without_write -w "done(weak_chain_beta_without_write)"  -N -B $HOME/.local/bin/jupyter nbconvert --ExecutePreprocessor.timeout=-1 --to notebook --execute WeakScalingReport.ipynb --output ../reports/WeakScalingReport_euler_beta_without_write$(date +%Y%m%d).ipynb
